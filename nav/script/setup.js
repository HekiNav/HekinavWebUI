...
function addZones(json) {
    ...
    labels.forEach(label => {
        var label = L.marker({lat: label.lat, lon: label.lng}, {interactive: false,
            icon: L.divIcon({
                className: 'label',
    ...
    L.geoJSON(json, {interactive: false,
        style: function () {
            return {fillColor: '#0000',
                    stroke: true,
                    weight: 6,
                    fill: true,
...
async function getDepartures(stop,stopPopup) {
    if (!depsRunning) {
        ...
        if (data) {
            ...
            } else if (data.data.stop.stoptimesWithoutPatterns.length > 0) {
                clearMap()
